...
class ViewCompiler {
    constructor() {
        ...
        this.__layoutPrefix = config.getPath('layoutsDir').replace(config.getPath('viewsDir'), '')
        this.__partialPrefix = config.getPath('partialsDir').replace(config.getPath('viewsDir'), '')
    ...
    async watch() {
        ...
        const watcher = chokidar.watch(config.getPath('viewsDir'), {
            awaitWriteFinish : {
    ...
    async __fileToViewObject(filename, forceLayout) {
        ...
        return this.__parseView({
            name : path.replace(/\.\w{1,10}$/, '').replace(config.getPath('viewsDir'), ''),
            template : content,
        }, forceLayout);
    }
    ...
    async __createFileFromView(view) {
        ...
        if (view.data.layout === 'none') {
            ...
            const layoutName = `${config.getPath('layoutsDir').replace(config.getPath('viewsDir'), '')}${view.data.layout}`
